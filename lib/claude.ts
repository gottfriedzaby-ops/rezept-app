...
- For ingredients WITHOUT a parenthetical metric value, convert non-metric measurements to metric units: g for grams, kg for kilograms, ml for millilitres, l for litres, cm for centimetres (convert cups, ounces, pounds, inches, Fahrenheit → Celsius accordingly). EXCEPTION: spoon measures (tablespoons/teaspoons) are NEVER converted — see the SPOON MEASURES rule below. This fallback never overrides the METRIC PRIORITY rule above
- SPOON MEASURES — preserve, never convert: when the source measures an ingredient in tablespoons or teaspoons, in any language or abbreviation (EL, Esslöffel, tbsp, tbs, tablespoon(s) → "EL"; TL, Teelöffel, tsp, teaspoon(s) → "TL"), keep the stated quantity and store the unit as "EL" or "TL". Do NOT convert spoon measures to ml or g. Store "3 EL Olivenöl" as amount: 3, unit: "EL". Store "1 TL Salz" as amount: 1, unit: "TL". Store "2 tbsp olive oil" as amount: 2, unit: "EL". Store "½ tsp baking soda" as amount: 0.5, unit: "TL". The only exception is the METRIC PRIORITY rule above: if the source line itself supplies a parenthetical metric value, use that source-provided metric instead. Prise is also a valid unit; store as amount: 1, unit: "Prise". Only the ingredient list/table is authoritative for unit and amount — do NOT derive amounts from EL/TL mentions inside step text
...
- STEP AMOUNTS — placeholders, never literal numbers: a cooking step's "text" must NEVER contain a literal ingredient quantity. Whenever a step refers to an amount of an ingredient that appears in that section's ingredient list (e.g. "200 g flour", "2 EL Öl", "half of the 500 g tomatoes", "add the 3 eggs"), replace BOTH the number/unit AND the ingredient noun with a placeholder of the form {{ingredient name}}. The text inside the braces must exactly match that ingredient's German "name" value from the ingredients list. The app expands {{Mehl}} at display time into the live, scaled quantity (e.g. "200 g Mehl"), so amounts stay correct when the user rescales the recipe. Examples: "Add 200 g flour and stir" → "{{Mehl}} unterrühren"; "Fold in half of the 200 g sugar" → "Die Hälfte von {{Zucker}} unterheben". A bare ingredient mention without any quantity (e.g. "season with the salt") stays as plain translated text — only quantified references become placeholders. Do NOT turn oven temperatures (°C), times/durations, servings counts, or tin/pan dimensions (cm) into placeholders — keep those as literal numbers.
...
3. Step quality: steps must be in logical cooking order, clearly written, include temperatures in °C, and include timerSeconds wherever a recipe would normally specify a time. If step text mentions non-metric units (cups, oz, °F) for cooking equipment or temperatures, add the metric equivalent in parentheses within that step's text. STEP INGREDIENT AMOUNTS: a step's text must NEVER contain a literal ingredient quantity. Whenever a step states an amount of an ingredient that is in the ingredient list (e.g. "200 g flour", "2 tbsp oil", "the 3 eggs"), replace the number+unit+ingredient-noun with a placeholder {{ingredient name}} whose inner text exactly matches that ingredient's "name" value. The app expands the placeholder into the current scaled amount at display time, so the recipe stays correct when rescaled. Example: "stir in 200 g flour" → "{{flour}} unterrühren" (use the ingredient's name; it will be translated together with the list in step 7). Leave oven temperatures, times, servings counts, and tin/pan dimensions as literal numbers — only ingredient amounts become placeholders.
