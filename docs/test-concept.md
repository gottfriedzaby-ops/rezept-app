...
| `__tests__/lib/amounts.test.ts` | `buildInlineAmountsPreamble`, `buildKnownAmountsPreamble`, `UNICODE_FRACTIONS` | Unit handling (EL/TL/Prise preserved as-is, lowercase normalised; l→ml; ml, g, kg unchanged); comma decimals; multi-line; preamble header text; zero/negative skip; parenthetical metric amounts; unicode fractions (½ ¼ ¾ ⅛); millilitres/gram spelling variants |
