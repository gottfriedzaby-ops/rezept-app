...
export default async function SharedCollectionPage({
  ...
  const [t, { data: share }] = await Promise.all([
      ...
      <div className="max-w-[1200px] mx-auto px-8 py-16">
        <header className="mb-6">
          ...
          <h1 className="font-serif text-5xl font-medium text-ink-primary tracking-[-0.02em] leading-tight">
            {share.label ? share.label : t("collectionFallback")}
          </h1>
        </header>

        <section>
          <p className="label-overline mb-8">{t("allRecipes")}</p>
          {!recipes || recipes.length === 0 ? (
            <p className="text-ink-secondary">{t("emptyCollection")}</p>
          ) : (
