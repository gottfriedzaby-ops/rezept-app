...
describe("WhatsNewPopup", () => {
  it("shows the latest release once on first visit and records the version", async () => {
    ...
    expect(
      await screen.findByText("Datenschutz-Einstellung & anonyme Statistiken")
    ).toBeInTheDocument();
    ...
    expect(
      screen.queryByText("Datenschutz-Einstellung & anonyme Statistiken")
  ...
  it("cumulates every release missed since the last seen version", async () => {
    ...
    // 1.6.0, 1.5.0, 1.4.2, 1.4.1, 1.4.0, 1.3.0 and 1.2.0 are all newer than 1.1.0.
    expect(
      await screen.findByText("Datenschutz-Einstellung & anonyme Statistiken")
    ).toBeInTheDocument();
    expect(
      screen.getByText("Vorgeschlagene Sammlungen")
    ).toBeInTheDocument();
    expect(
    ...
    expect(
      screen.getByText("Versionshinweise & Feinschliff")
    ).toBeInTheDocument();
    expect(
